...
/* TO DO: sort out syncing of lines, i.e must determing the maximum x/y of points to det bounding box */
void
                 ...
                 sync sync_mode, bool primary, action_struct * payload)
{
    ...
    /* TO DO: */
    /* below is NOT good, because what if center is off screen but radius is large enough for an arc to
       appear on image? */
    /*    if(!bound_by_rect(x1, y1, 0, 0, tex->width, tex->height)) return; */
            ...
            while(y1 < tex->height  && cmp_color(old_color, get_pixel_color(tex, x, y1)) )
                {
                    ...
                    /* got rid of the - 1 from both tex->width below */
                    ...
                    else if(spanRight && x < tex->width - 1 && !cmp_color(old_color,
                                                                          get_pixel_color(tex, x + 1, y1)))
                        {
                            spanRight = false;
                        } 
